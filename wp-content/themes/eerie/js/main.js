...
$(document).ready(function(){
    ...
    $('body').on('click touchend', '.overlay',function(){
        ...
        pathEl.stop().animate( { 'path' : newPath }, 0, mina.elastic,function(){
    ...
    $('.menu').swipe( {
        swipeStatus:function(event, phase, direction, distance, duration, fingers) {
            ...
            if(phase=="move" && !$('.menu').hasClass('open') ){
                ...
                newPath = 'M0,0C0,0,0,'+menuH+',0,'+menuH+'C0,'+menuH+',0,'+menuH+',0,'+menuH+'C'+ (menuW*decX/60) +','+(menuH*(decY-20)/150)+','+(menuW*decX/60)+','+(menuH*(decY+20)/150)+',0 0C0,0,0,0,0,0C0,0,0,0,0,0';
                $('.bg path').attr('d', newPath);
                // pathEl.stop().animate( { 'path' : newPath }, 500, mina.elastic );
                if(decX>60){
                    newPath = 'M 0 0, L 0 '+menuH+', L '+menuW+' '+menuH+', L '+menuW+' 0, Z';
                    pathEl.stop().animate( { 'path' : newPath }, 1000, mina.elastic );
                    $('.menu').addClass('open');
            ...
            if(phase=="end"){
                ...
                if(decX<60){
                    ...
                    newPath = 'M 0 0, L 0 '+menuH+', L '+menuW+' '+menuH+', L '+menuW+' 0, Z';
                    pathEl.stop().animate( { 'path' : newPath }, 600, mina.elastic );
                    $('.menu').addClass('open');
                    $('.menu').removeClass('touch');
                }
            }
